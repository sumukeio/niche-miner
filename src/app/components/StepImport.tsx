...
export default function StepImport({ onNext }: { onNext: (projectId: string) => void }) {
  ...
  const handleFileUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      // 1. 创建项目
      const projectName = file.name.replace('.csv', '') + '_' + new Date().toLocaleDateString()
      ...
      // 2. 解析 CSV (使用数组模式，不自动识别表头)
      setStatusText('正在解析 CSV 文件...')
      
      Papa.parse<string[]>(file, {
        header: false, // <--- 改成 false，我们要自己找表头
        skipEmptyLines: true,
        encoding: 'GB18030', // 保持中文编码
        ...
        complete: async (results) => {
          ...
          for (let i = 0; i < Math.min(5, rawRows.length); i++) {
            if (rawRows[i] && rawRows[i].some(cell => cell.includes('关键词'))) {
              headerRowIndex = i
              break
            }
          }
          ...
          if (headerRowIndex === -1) {
            alert('错误：找不到“关键词”列。请确认这是正确的 CSV 文件。')
            setLoading(false)
            return
          }

          // 建立列索引映射 (比如: "关键词"在第0列, "PC检索量"在第9列)
    ...
    <div className="flex flex-col items-center justify-center p-12 border-2 border-dashed border-slate-300 rounded-xl bg-slate-50 hover:bg-slate-100 transition-colors">
      ...
      {!loading ? (
        <>
          ...
          <h3 className="text-xl font-semibold text-slate-800 mb-2">上传 5118 长尾词 CSV</h3>
          <p className="text-slate-500 mb-6 text-center max-w-sm">
            支持自动识别 5118 表头偏移格式。<br/>
            一次可处理 10 万行数据。
          </p>
